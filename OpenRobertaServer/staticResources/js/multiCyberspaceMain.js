require(['require', 'jquery', 'wrap', 'comm', 'user.model', 'program.model', 'robot.model', 'guiState.model', 'interpreter.jsHelper', 'external/MultiCyberspace'], function (require) {
    //
    // Global definitions
    //
    $ = require('jquery');
...
function handleQuery() {
    ...
    if (target.length >= 2) {
        var programIDListString = target[1];
        var secret = target.length >= 3 ? target[2] : "";
 ...
 */
function init() {
    GUISTATE_MODEL.init().then(function (res) {
