...
define(["require", "exports", "./Random"], function (require, exports, Random_1) {
    ...
    exports.SimUtil = exports.asUniqueArray = void 0;
    var asUniqueArray = function (a) { return a; };
    exports.asUniqueArray = asUniqueArray;
    function applyRestrictedKey(type, key) {
        return type[key];
    }
    var SimUtil = /** @class */ (function () {
        function SimUtil() {
        }
        SimUtil.safeIndexing = function (value, index) {
            if (value == undefined) {
                return undefined;
            }
            else {
         ...
         */
        SimUtil.deepEqual = function (x, y) {
            return (x && y && typeof x === 'object' && typeof y === 'object') ?
                (Object.keys(x).length === Object.keys(y).length) &&
                    Object.keys(x).reduce(function (isEqual, key) {
                        return isEqual && SimUtil.deepEqual(x[key], y[key]);
                    }, true) : (x === y);
        };
        /**
         ...
         */
        SimUtil.closedRange = function (start, end, step) {
         ...
         * @param step the step size of the range
         */
        SimUtil.range = function (start, end, step) {
            if (step === void 0) { step = 1; }
            var ans = [];
            for (var i = start; i < end; i += step) {
                ans.push(i);
         ...
         */
        SimUtil.toDegrees = function (radians) {
            return radians / Math.PI * 180;
        };
        /**
         * Convert degrees to radians
         */
        SimUtil.toRadians = function (degrees) {
         ...
         */
        SimUtil.continuousSign = function (x, width) {
            if (Math.abs(x) >= width) {
                return Math.sign(x);
            }
            return x / width;
         ...
         */
        SimUtil.exhaustiveSwitch = function (value) {
         ...
         * const value = "C"
         * if (SimUtil.listIncludesValue(list, value)) {
         *     value // is of type "A" | "B"
         * }
         */
        SimUtil.listIncludesValue = function (list, value) {
         ...
         */
        SimUtil.flatMapOptional = function (value, mapping) {
            if (value == undefined) {
                return undefined;
            }
            else {
         ...
         */
        SimUtil.genUid = function () {
            var length = 20;
            var soupLength = SimUtil.soup_.length;
            var id = [];
            for (var i = 0; i < length; i++) {
                id[i] = SimUtil.soup_.charAt(Math.random() * soupLength);
            }
            return id.join('');
        };
        ;
         ...
         * @return {string} A globally unique ID string.
         */
        SimUtil.genHtmlUid = function () {
            var length = 25;
            var soupLength = SimUtil.soupHTML_.length;
            var id = [];
            id[0] = SimUtil.soupAlphabet_.charAt(Math.random() * SimUtil.soupAlphabet_.length);
            for (var i = 1; i < length; i++) {
                id[i] = SimUtil.soupHTML_.charAt(Math.random() * soupLength);
            }
            return id.join('');
        };
        ;
        SimUtil.genHtmlUid2 = function () {
         ...
         */
        SimUtil.checksumFNV32 = function (array, previousChecksum) {
        ...
        SimUtil.flattenArray = function (array) {
            var result = [];
        ...
        SimUtil.pushAll = function (array, otherArray) {
            for (var i = 0; i < otherArray.length; i++) {
                array.push(otherArray[i]);
            }
        };
        SimUtil.map2D = function (array, mapping) {
            return array.map(function (subArray) { return subArray.map(mapping); });
        };
        SimUtil.reshape1Dto2D = function (array, maxSubArrayLength) {
            var newArray = [];
            var temp = [];
        ...
        SimUtil.reshape2D = function (array, maxSubArrayLength) {
            var newArray = [];
            var temp = [];
            var l1 = array.length;
            for (var i = 0; i < l1; i++) {
         ...
         */
        SimUtil.anyTuples = function (list) {
            var e_1, _a;
            if (list.length == 0) {
                return list;
            }
                ...
                var _loop_1 = function (value) {
                    var val = SimUtil.anyTuples(list.slice(0, -1)).map(function (tuple) {
                        return tuple.concat(value);
                    });
                    SimUtil.pushAll(result, val);
                };
                try {
         ...
         * i.e.`[[list[0][0],list[1][0], list[2][0],...], [list[0][1],list[1][0], list[2][0],...], ...]`
         */
        SimUtil.tuples = function (list) {
            return SimUtil.anyTuples(list);
        };
        SimUtil.strictPropertiesTuples = function (type, keys) {
            ...
            var tuples = SimUtil.anyTuples(values);
        ...
        SimUtil.propertiesTuples = function (type, keys) {
            for (var i = 0; i < keys.length; i++) {
                for (var j = i + 1; j < keys.length; j++) {
                    if (keys[i] === keys[j]) {
                        console.error("The property name (" + keys[i] + ") is duplicate");
        ...
        SimUtil.allPropertiesTuples = function (type) {
            var keys = Object.keys(type);
            return SimUtil.propertiesTuples(type, keys);
        };
        /**
        ...
        */
        SimUtil.generateMultiSetTuples = function (values, length) {
            ...
            var repeatedValues = values.map(function (value) {
                return SimUtil.closedRange(0, length).map(function (len) {
         ...
         */
        SimUtil.toHHMMSS = function (time) {
            var sec_num = time;
         ...
         */
        SimUtil.toTimeString = function (time) {
            var e_2, _a;
            var sec = time;
        ...
        SimUtil.nonNullObjectValues = function (value) {
            var e_3, _a;
        ...
        SimUtil.mapNotNull = function (array, transform) {
            var e_4, _a;
            var result = [];
            try {
        ...
        SimUtil.getOptions = function (init, someOptions) {
         ...
         */
        SimUtil.removeFromArray = function (array, element) {
        ...
        SimUtil.stringReplaceAll = function (string, searchValue, replacement) {
            return string.split(searchValue).join(string);
        };
        SimUtil.vectorEqual = function (v1, v2) {
            return v1.x === v2.x && v1.y === v2.y;
        };
        SimUtil.vectorAdd = function (v1, v2) {
            return { x: v1.x + v2.x, y: v1.y + v2.y };
        };
        SimUtil.vectorSub = function (v1, v2) {
            return { x: v1.x - v2.x, y: v1.y - v2.y };
        };
        SimUtil.vectorDistance = function (v1, v2) {
            var dx = v1.x - v2.x;
            var dy = v1.y - v2.y;
            return Math.sqrt(dx * dx + dy * dy);
        };
        SimUtil.vectorDistanceSquared = function (v1, v2) {
            var dx = v1.x - v2.x;
            var dy = v1.y - v2.y;
            return dx * dx + dy * dy;
        };
         ...
         */
        SimUtil.getPixelRatio = function () {
            return window.devicePixelRatio || 0.75; // 0.75 is default for old browsers
        };
        SimUtil.cloneVector = function (value) {
            return { x: value.x, y: value.y };
        };
        SimUtil.randomElement = function (array) {
            if (array.length == 0) {
                return undefined;
            }
            else {
        ...
        SimUtil.getRootURL = function (ignorePort) {
        ...
        SimUtil.soupAlphabet_ = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
        SimUtil.soupHTML_ = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_:';
        SimUtil.idNumber = 0;
        return SimUtil;
    }());
    exports.SimUtil = SimUtil;
});
